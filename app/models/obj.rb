class Obj < ApplicationRecord
	...
	validates :datebought, format: { with: /\d{4}-\d{2}-\d{2}/, message: "datebought: wrong format" }
	validates :dateretire, format: { with: /\d{4}-\d{2}-\d{2}/, message: "dateretire: wrong format" }
